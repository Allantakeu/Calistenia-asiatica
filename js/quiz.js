    ...
    setTimeout(() => {
        ...
        switch (stepNumber) {
            case 1:
                html = `
                    <div class="step-content">
                        <p class="subtitle">TESTE DE 1 MINUTO</p>
                        <h2 class="question">Quantos anos você tem?</h2>
                        <div class="options-list">
                            ${['39 – 45', '46 – 50', '51 – 60', '60+'].map(opt => 
                                `<button class="option-btn" onclick="setAnswer('ageRange', '${opt}')">${opt}</button>`
                            ).join('')}
                        </div>
                    </div>`;
                break;
            case 2:
                html = `
                    <div class="splash-screen">
                        <h2>Elas conseguiram e você também pode</h2>
                        <p>Mais de 27 milhões de mulheres já transformaram suas vidas com a Calistenia Asiática</p>
                        <img src="assets/hero-bg.jpg" alt="Transformação" style="width:100%; border-radius: 10px; margin: 20px 0;" />
            ...
            case 3:
                html = `
                    <div class="step-content">
                        <h2 class="question">Você já experimentou exercícios de calistenia?</h2>
                        <div class="options-list">
                            <button class="option-btn" onclick="setAnswer('calistheniaExperience', 'sim')">Sim</button>
                            <button class="option-btn" onclick="setAnswer('calistheniaExperience', 'nao')">Não</button>
                        </div>
                    </div>`;
                break;
            case 4:
                ...
                html = `
                    <div class="splash-screen conditional-splash">
                        <h2>${heading4}</h2>
                        <p>${text4}</p>
            ...
            case 5:
                const goals = [
                    { t: 'Perder peso', i: '⚖️' },
                    { t: 'Desenvolver músculos', i: '💪' },
                    { t: 'Manter o peso e ficar em forma', i: '🧘' },
                    { t: 'Melhorar a aptidão física', i: '⚡' }
                ];
                html = `
                    <div class="step-content">
                        ...
                        <div class="options-list icon-list">
                            ${goals.map(opt => 
                                `<button class="option-btn" onclick="setAnswer('mainGoal', '${opt.t}')"><span class="icon">${opt.i}</span> ${opt.t}</button>`
                            ).join('')}
                        </div>
                    </div>`;
                break;
            case 6:
                const addGoals = [
                    { t: 'Aumentar a energia', i: '🔋' },
                    { t: 'Melhorar o sono', i: '🌙' },
                    { t: 'Reduzir o estresse', i: '🧠' },
                    { t: 'Melhorar a postura e a mobilidade', i: '🚶♀️' },
                    { t: 'Desenvolver flexibilidade', i: '✨' }
                ];
                html = `
                    <div class="step-content">
                        ...
                        <div class="options-list multi-select" id="additionalGoals-list">
                            ${addGoals.map(opt => {
                                ...
                                return `<button class="option-btn ${selected}" onclick="toggleMulti('additionalGoals', '${opt.t}', this)"><span class="icon">${opt.i}</span> ${opt.t}</button>`
                            }).join('')}
                        </div>
                        <button class="primary-btn mt-4" onclick="nextStep()">Continuar</button>
                    </div>`;
                ...
                html = `
                    <div class="step-content">
                        ...
                        <div class="options-list multi-select">
                            <button class="option-btn" onclick="toggleMultiAll('bodyZones', this)">Melhorar o corpo inteiro</button>
                            ${zones.map(opt => {
                                const selected = quizState.answers.bodyZones.includes(opt) ? 'selected' : '';
                                return `<button class="option-btn ${selected}" onclick="toggleMulti('bodyZones', '${opt}', this)">${opt}</button>`
                            }).join('')}
                        </div>
                    ...
                    <div class="step-content">
                        ...
                        <div class="options-list multi-select">
                            ${targets.map(opt => {
                                ...
                                return `<button class="option-btn ${selected}" onclick="toggleMulti('targetZones', '${opt}', this)">${opt}</button>`
                            }).join('')}
                        </div>
                        <button class="primary-btn mt-4" onclick="nextStep()">Próximo passo</button>
                    </div>`;
            ...
            case 9:
                html = `
                    <div class="splash-screen edu-splash">
                        <h2>Apenas 7 minutos por dia — transformarão seu corpo e destravarão o seu metabolismo sem o esforço exaustivo da academia!</h2>
                        <p>O Protocolo de Calistenia Asiática utiliza ativações de fibras profundas para derreter a gordura acumulada e chapar a barriga, agindo onde os exercícios comuns de academia não conseguem chegar, de forma simples e definitiva...</p>
                        <button class="primary-btn mt-4" onclick="nextStep()">Continuar</button>
                    </div>`;
                break;
            case 10:
            ...
            case 12:
                ...
                html = `
                    <div class="step-content">
                        <h2 class="question">Há quanto tempo você estava na melhor forma da sua vida?</h2>
                        <div class="options-list">
                            ${fHist.map(opt => `<button class="option-btn" onclick="setAnswer('fitnessHistory', '${opt}')">${opt}</button>`).join('')}
                        </div>
                    </div>`;
                break;
            case 13:
            ...
            case 14:
                const comforts = [
                    { t: 'Sinto dor/Desconforto', i: '😔', v: 'dor' },
                    { t: 'Sinto alguma dificuldade', i: '😐', v: 'dificuldade' },
                    { t: 'Sinto-me confortável', i: '😊', v: 'confortavel' }
                ];
                html = `
                    <div class="step-content">
                        <h2 class="question">Você se sente confortável ao se exercitar?</h2>
                        <div class="options-list">
                            ${comforts.map(opt => `<button class="option-btn" onclick="setAnswer('comfortLevel', '${opt.v}')"><span class="icon">${opt.i}</span> ${opt.t}</button>`).join('')}
                        </div>
                    </div>`;
                break;
            case 15:
            ...
            case 20: // Lifestyle
                const lifestyles = [
                    { t: 'Sedentário', d: 'Eu passo a maior parte do dia sentado', i: '🛋️' },
                    { t: 'Atividade moderada', d: 'Eu faço pausas ativas', i: '🚶' },
                    { t: 'Eu sou imparável', d: 'Estou de pé o dia todo', i: '🏃' }
                ];
                html = `
                    <div class="step-content">
                        ...
                        <div class="options-list">
                            ${lifestyles.map(opt => `
                                <button class="option-btn with-desc" onclick="setAnswer('lifestyle', '${opt.t}')">
                                    <div class="opt-main"><span class="icon">${opt.i}</span> <span>${opt.t}</span></div>
                    ...
                    <div class="step-content">
                        ...
                        <div class="options-list multi-select">
                            ${meals.map(opt => {
                                ...
                                return `
                                <button class="option-btn with-desc ${selected}" onclick="toggleMulti('mealPreferences', '${opt.t}', this)">
                                    <div class="opt-main">${opt.t}</div>
                                    <div class="opt-desc">${opt.d}</div>
                                </button>
                                `
                            }).join('')}
                    ...
                    <div class="step-content">
                        ...
                        <div class="options-list multi-select">
                            ${extras.map(opt => {
                                ...
                                return `
                                <button class="option-btn with-desc ${selected}" onclick="toggleMulti('habits', '${opt.t}', this)">
                                    <div class="opt-main">${opt.t}</div>
                                    <div class="opt-desc">${opt.d}</div>
                                </button>
                                `
            ...
            case 34: // Profile Ready BMI
                ...
                html = `
                    <div class="step-content bmi-step">
                        ...
                        <!-- Warning Box -->
                        <div style="background:#fef2f2; border:1px solid #fee2e2; border-radius:12px; padding:15px; margin-bottom:15px; text-align:left; display:flex; gap:10px;">
            ...
            case 35: // Summary / Plan Summary
                ...
                html = `
                    <div class="step-content summary-step">
                        ...
                        <div style="background:#f0fdf4; border:1px solid #bbf7d0; border-radius:12px; padding:15px; margin-bottom:20px; text-align:left; font-size:13px; color:#166534; font-weight:500;">
                            ✨ De acordo com suas respostas, você tem 98% de chance de atingir sua meta seguindo nosso cronograma de 21 dias.
                        </div>
            ...
            case 36: // Offer VSL
                ...
                html = `
                    <div class="offer-screen" style="padding-top: 10px;">
                        <!-- Sticky Banner Top -->
                        <div id="sticky-countdown-banner" style="display: none; background: #d64d7a; color: white; padding: 10px; font-weight: bold; position: fixed; top: 0; left: 0; right: 0; z-index: 1000; text-align: center; font-size: 14px; box-shadow: 0 2px 8px rgba(0,0,0,0.15);">
                            ⏱️ A oferta especial expira em: <span id="countdown-timer">10:00</span>
                        </div>
                        ...
                        <div class="vsl-container" style="margin: 20px auto; background: #000; padding-bottom: 177.77%; position: relative; width: 100%; max-width: 320px; border-radius: 16px; overflow: hidden; box-shadow: 0 6px 20px rgba(0,0,0,0.2); border: 2px solid #e2e8f0;">
                            <iframe src="https://player-vz-b8de2e87-683.tv.pandavideo.com.br/embed/?v=79ca83eb-0870-49c5-9792-362896cc7d20" style="border:0; position:absolute; top:0; left:0; width:100%; height:100%;" allow="accelerometer; gyroscope; autoplay; encrypted-media; picture-in-picture" allowfullscreen="true"></iframe>
                        </div>
                        ...
                        <div id="delayed-offer" style="display: none; margin-top: 25px;">
                            ...
                            <div style="background:#fff; border-radius:16px; padding:25px; box-shadow:0 6px 18px rgba(0,0,0,0.06); border:2px solid #d64d7a; margin-bottom:20px; text-align:center;">
                                <div style="display:flex; align-items:center; justify-content:center; gap:8px; font-weight:600; color:#ea580c; font-size:12px; margin-bottom:15px;">
                                    ⚠️ Restam apenas <span id="vagas-count-2">7</span> vagas com desconto especial
                                </div>
                                ...
                                <p style="font-size:11px; color:#aaa; margin-top:10px;">🔒 Compra 100% Segura • 30 Dias de Garantia Incondicional</p>
                            </div>
